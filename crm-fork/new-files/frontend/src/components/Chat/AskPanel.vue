  ...
  >
    ...
    >
      ...
      <div class="flex items-center gap-1">
        ...
        <button
          class="p-1 text-ink-gray-5 hover:text-ink-gray-8"
          @click="close"
        >
    ...
    <div ref="scroller" class="flex-1 space-y-4 overflow-y-auto px-4 py-4">
      ...
      <div v-if="!turns.length" class="pt-6">
        ...
        <div class="mb-4 text-xs text-ink-gray-5">
          {{
            __(
              'Ask about your leads, deals, contacts or tasks in plain English.',
            )
          }}
        </div>
        <button
      ...
      <div v-for="(t, i) in turns" :key="i">
        ...
        <template v-else>
          ...
          <div
            v-if="t.rows?.length"
            class="overflow-hidden rounded-md border border-outline-gray-2"
          >
            ...
            <pre
              ...
              >{{ JSON.stringify(t.query, null, 1) }}</pre
            >
          </div>

          <div v-else-if="t.doctype" class="text-xs text-ink-gray-5">
    ...
    <div class="border-t border-outline-gray-2 p-3">
      <div
        class="flex items-end gap-2 rounded-lg border border-outline-gray-2 bg-surface-gray-1 px-2 py-1.5"
      >
        <textarea
          v-model="draft"
          rows="1"
...
async function ask(text) {
  ...
  if (!isReady(credentialId.value)) {
    Object.assign(turn, {
      pending: false,
      error: __('Choose or configure an AI key for this browser first.'),
    })
    busy.value = false
    return
  }
