<template>
  <div class="flex flex-col gap-4">
    ...
    <div>
      <div class="mb-1 text-p-sm text-ink-gray-7">{{ __('What it should do') }}</div>
      <textarea
        ...
        :placeholder="__('You look after new leads. Message them, find out what they need, and book a call.')"
      />
    ...
    <div class="rounded-md bg-surface-gray-2 px-3 py-2 text-p-sm text-ink-gray-6">
      {{ __('It can only ever use the connectors on the canvas — that limit is enforced in code, not by asking nicely.') }}
    </div>

    <div class="border-t border-outline-gray-1 pt-3">
      <div class="mb-2 text-p-sm font-medium text-ink-gray-7">{{ __('Model') }}</div>
      <!--
        A credential is chosen here and configured in Settings. Offering to
        create one inline would mean an API-key field on a page about behaviour,
        which is how keys end up in the wrong place.
      -->
      <Select
        v-model="bot.ai_model"
        :options="[{ label: __('Default for conversations'), value: '' },
    ...
    <div class="grid grid-cols-2 gap-3 border-t border-outline-gray-1 pt-3">
      <div>
        ...
        <Select v-model="bot.channel"
          :options="[{label: 'WhatsApp', value: 'WhatsApp'},
                     {label: 'Email', value: 'Email'},
                     {label: __('Nothing'), value: 'None'}]" />
      </div>
      <FormControl v-model.number="bot.max_steps" type="number" :label="__('Max steps')"
