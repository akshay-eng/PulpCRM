  ...
  <div class="flex flex-col gap-4">
    <div class="flex items-start gap-2.5">
      <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-surface-gray-2">
        <component :is="connectorIcon(spec.icon)" class="h-4 w-4 text-ink-gray-7" />
      </div>
      <div class="min-w-0">
        <div class="text-p-base font-medium text-ink-gray-8">{{ spec.label }}</div>
        <div class="text-p-sm text-ink-gray-5">{{ spec.description }}</div>
      </div>
    </div>

    <!-- Credential: chosen here, configured in Settings. -->
    <div v-if="spec.credential"
      class="rounded-lg border px-3 py-2.5"
      :class="spec.credential.configured
        ? 'border-outline-gray-2 bg-surface-white'
        : 'border-amber-200 bg-amber-50'">
      <div class="flex items-center gap-2">
        <component :is="spec.credential.configured ? LucideCheckCircle : LucideTriangleAlert"
          class="h-4 w-4 shrink-0"
          :class="spec.credential.configured ? 'text-green-600' : 'text-amber-600'" />
        <div class="flex-1 text-p-base text-ink-gray-8">{{ spec.credential.label }}</div>
      </div>
      <div class="mt-1 text-p-sm" :class="spec.credential.configured ? 'text-ink-gray-5' : 'text-amber-700'">
      ...
      <Button class="mt-2 w-full" :label="spec.credential.configured
          ? __('Change it in Settings') : __('Set it up in Settings')"
        @click="openSettings">
    ...
    <div v-for="f in spec.config || []" :key="f.field">
      ...
      <Select v-if="f.type === 'select'" :model-value="value(f)"
        :options="(f.options || []).map((o) => ({ label: o, value: o }))"
        @update:model-value="set(f, $event)" />
      <Select v-else-if="f.type === 'sender'" :model-value="value(f)"
        :options="[{ label: __('The default mailbox'), value: '' },
                   ...senders.map((a) => ({ label: a.email_id, value: a.email_id }))]"
        @update:model-value="set(f, $event)" />
      <Select v-else-if="f.type === 'availability'" :model-value="value(f)"
        :options="[{ label: __('Pick automatically'), value: '' },
                   ...availabilities.map((a) => ({ label: a, value: a }))]"
        @update:model-value="set(f, $event)" />
      <FormControl v-else-if="f.type === 'int'" type="number" :model-value="value(f)"
      ...
      />
      <FormControl v-else type="text" :model-value="value(f)" :placeholder="f.placeholder"
        @update:model-value="set(f, $event)" />
      <div v-if="f.help" class="mt-1 text-p-sm text-ink-gray-5">{{ f.help }}</div>
    </div>
    ...
    <div class="border-t border-outline-gray-1 pt-3">
      ...
      <div v-for="t in spec.tools" :key="t.name" class="flex gap-2 py-1">
        ...
        <div>
          ...
          <div class="text-p-sm leading-snug text-ink-gray-5">{{ t.description }}</div>
        </div>
      </div>
    </div>
    ...
    <div class="flex items-center justify-between border-t border-outline-gray-1 pt-3">
      ...
      <Switch :model-value="Boolean(node.enabled)"
        @update:model-value="node.enabled = $event ? 1 : 0" />
    </div>

    <Button theme="red" variant="subtle" :label="__('Remove this connector')"
      @click="$emit('remove')" />
  </div>
</template>
