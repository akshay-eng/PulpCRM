<template>
  <SettingsLayoutBase
    ...
    :description="
      __(
        'When meetings can be booked, and how long they run. Slots are only ever offered inside these hours.',
      )
    "
  >
    <template #content>
      ...
      <template v-else>
        ...
        >
          <div class="mb-4 grid grid-cols-2 gap-3">
            <FormControl
              v-model="availability.user"
              type="text"
              :label="__('For user')"
              :placeholder="__('Blank = everyone')"
            />
            <FormControl
              v-model="availability.timezone"
              type="text"
              :label="__('Timezone')"
            />
            <FormControl
              v-model="availability.holiday_list"
              type="select"
              :label="__('Holiday list')"
              :options="['', ...config.holiday_lists]"
            />
            <FormControl
              v-model.number="availability.slot_minutes"
              type="number"
              :label="__('Slot every (minutes)')"
            />
            <FormControl
              v-model.number="availability.min_notice_minutes"
              type="number"
              :label="__('Minimum notice (minutes)')"
            />
            <FormControl
              v-model.number="availability.max_days_ahead"
              type="number"
              :label="__('Look ahead (days)')"
            />
            <FormControl
              v-model.number="availability.buffer_after_minutes"
              type="number"
              :label="__('Gap after each meeting')"
            />
            <FormControl
              v-model.number="availability.max_bookings_per_day"
              type="number"
          ...
          >
            ...
            <template v-if="hoursFor(day)">
              <FormControl
                v-model="hoursFor(day).start_time"
                type="time"
                class="w-[130px]"
              />
              <span class="text-ink-gray-5">{{ __('to') }}</span>
              <FormControl
                v-model="hoursFor(day).end_time"
                type="time"
                class="w-[130px]"
              />
            </template>
        ...
        <div
          v-if="slots.length"
          class="mt-4 rounded-lg border border-outline-gray-2 p-3"
        >
        ...
        <div v-else-if="previewed" class="mt-4 text-p-sm text-ink-amber-3">
        ...
        <div class="mt-8">
          ...
          <div
            ...
            class="flex items-center gap-3 border-b border-outline-gray-1 py-2 last:border-0"
          >
            <span class="flex-1 text-p-base text-ink-gray-8">{{
              s.service_name
            }}</span>
            <span class="text-p-sm text-ink-gray-5"
              >{{ s.duration_minutes }} {{ __('min') }}</span
            >
...
const availability = computed(
  () =>
    config.value.availabilities.find((a) => a.name === selectedName.value) ||
    null,
)
