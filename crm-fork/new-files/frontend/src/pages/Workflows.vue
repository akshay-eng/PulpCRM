    ...
    <div class="mx-auto max-w-3xl">
      ...
      >
        ...
        <div class="max-w-sm text-center text-p-base">
          {{
            __(
              'Draw the steps once and they happen the same way every time — when a lead arrives, when a deal moves, on a schedule.',
            )
          }}
        </div>
        <Button
      ...
      <div v-else class="flex flex-col gap-2">
        ...
        >
          <div class="flex items-center gap-3">
            <AutomationAvatar
              :identity="w.name || w.workflow_name"
              kind="workflow"
            />
          ...
          <div
            v-if="w.description"
            class="mt-1 pl-11 text-p-sm text-ink-gray-6"
          >
            {{ w.description }}
          </div>
        </div>
...
const open = (w) =>
  router.push({ name: 'Workflow', params: { workflowId: w.name } })
...
async function create() {
  ...
  try {
    ...
    toast.error(
      e.messages?.[0] || e.message || __('Could not create the workflow'),
    )
  } finally {
...
async function remove(w) {
  ...
  if (
    !window.confirm(
      __('Delete “{0}”? Its run history goes too.', [w.workflow_name]),
    )
  )
    return
  try {
