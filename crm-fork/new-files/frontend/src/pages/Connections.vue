<template>
  <LayoutHeader>
    <template #left-header>
      <Breadcrumbs
        :items="[{ label: __('Connections'), route: { name: 'Connections' } }]"
      />
    </template>
    ...
    <div class="mx-auto max-w-4xl">
      ...
      >
        ...
        <div class="mb-3 text-sm text-ink-gray-5">
          {{
            __(
              'Which bridge Baton sends through. Only one is active at a time.',
            )
          }}
        </div>
        <div class="flex gap-3">
          <button
            ...
            :class="
              data.active_channel === opt.id
                ? 'border-orange-400 bg-surface-orange-1 ring-1 ring-orange-400/40'
          ...
          >
            <div class="flex items-center gap-2">
              ...
              <Badge
                v-if="data.active_channel === opt.id"
                theme="orange"
                variant="subtle"
              >
                {{ __('Active') }}
        ...
        <div
          class="mt-4 flex items-center gap-3 border-t border-outline-gray-1 pt-3"
        >
      ...
      >
        ...
        >
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs text-ink-gray-5">{{
              __('self-hosted · unofficial')
            }}</span>
          </div>
          <Badge :theme="openwaStatus.theme" variant="subtle">{{
            openwaStatus.label
          }}</Badge>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 px-4 py-4">
          ...
          <FormControl
            ...
            type="password"
            :label="
              data.openwa?.has_api_key
                ? __('API key (stored — leave blank to keep)')
                : __('API key')
            "
            placeholder="owa_…"
          />
          <div class="col-span-2">
            ...
            <div class="flex gap-2">
              ...
              <Button
                :label="__('Fetch sessions')"
                :loading="fetching"
                @click="fetchSessions"
              />
            </div>
            <div v-if="sessionError" class="mt-1 text-xs text-ink-red-4">
              {{ sessionError }}
            </div>
          </div>
        </div>
        ...
        <div
          class="flex items-center gap-2 border-t border-outline-gray-2 px-4 py-3"
        >
          <Button
            variant="solid"
            :label="__('Save')"
          ...
          <span
            v-if="testResult"
            class="text-xs"
            :class="testResult.ok ? 'text-ink-green-3' : 'text-ink-red-4'"
          >
            {{ testResult.msg }}
          </span>
        </div>
        <div
          v-if="webhookNote"
          class="border-t border-outline-gray-1 px-4 py-2 text-xs text-ink-gray-5"
        >
          {{ webhookNote }}
        </div>
      </div>
      ...
      <div class="rounded-lg border border-outline-gray-2 bg-surface-white">
        ...
        <div
          v-if="!data.meta?.installed"
          class="px-4 py-4 text-sm text-ink-gray-6"
        >
          {{
            __('The official WhatsApp channel is not installed on this site.')
          }}
        </div>
        ...
        <template v-else>
          <div class="grid grid-cols-2 gap-4 px-4 py-4">
            <FormControl
              v-model="meta.phone_id"
              type="text"
              :label="__('Phone number ID')"
            />
            <FormControl
              v-model="meta.business_id"
              type="text"
            ...
            <FormControl
              ...
              type="password"
              :label="
                metaAccount?.has_token
                  ? __('Access token (stored — blank keeps it)')
                  : __('Access token')
              "
              placeholder="EAA…"
            />
            <FormControl
              v-model="meta.app_secret"
              type="password"
              :label="
                data.meta?.has_app_secret
                  ? __('App secret (stored — blank keeps it)')
                  : __('App secret')
              "
            />
            <div class="flex items-end">
              <div class="text-xs text-ink-gray-5">
                {{
                  __(
                    'The app secret verifies inbound webhook signatures. Without it Baton refuses the webhook.',
                  )
                }}
              </div>
            </div>
          ...
          >
            ...
            <span
              v-if="metaResult"
              class="text-xs"
              :class="metaResult.ok ? 'text-ink-green-3' : 'text-ink-red-4'"
            >
              {{ metaResult.msg }}
            </span>
          </div>
      ...
      <div
        class="mt-4 rounded-md bg-surface-gray-2 px-4 py-3 text-xs text-ink-gray-6"
      >
        <b>{{ __('Which should you use?') }}</b>
        {{
          __(
            'Meta is official and durable, but cannot see messages you type on your own phone — so human-handoff detection is partial. OpenWA rides your real account and sees everything, which makes handoff exact, but it is unofficial and carries account-ban risk.',
          )
        }}
      </div>
    </div>
...
async function switchChannel(id) {
  data.value = await call('baton.api.connections.set_active_channel', {
    channel: id,
  })
  toast.success(
    __('WhatsApp now goes through {0}', [id === 'openwa' ? 'OpenWA' : 'Meta']),
  )
}
...
async function fetchSessions() {
  ...
  try {
    ...
    if (!wa.session_id && r.sessions.length === 1)
      wa.session_id = r.sessions[0].id
  } finally {
...
async function registerHook() {
  ...
  try {
    ...
    const r = await call('baton.api.connections.register_openwa_webhook', {
      public_url: s.url,
    })
...
async function saveMeta() {
  ...
  try {
    data.value = await call('baton.api.connections.save_meta', {
      ...
      app_secret: meta.app_secret || undefined,
    })
    meta.token = ''
    meta.app_secret = ''
    toast.success(__('Saved'))
...
async function testMeta() {
  ...
  try {
    const r = await call('baton.api.connections.test_meta', {
      account: metaAccount.value?.name,
    })
    metaResult.value = r.ok
      ? {
          ok: true,
          msg: __('{0} ({1})', [r.display_name || '', r.number || '']),
        }
